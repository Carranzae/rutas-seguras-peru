...
export default function VerificationsPage() {
    ...
    // Ghoscloud Deep Scan
    const performDeepScan = async (overrideInput?: string, overrideMode?: 'dni-phys' | 'dni-virt' | 'name' | 'phone' | 'background') => {
        const queryToUse = overrideInput || scanInput;
        const modeToUse = overrideMode || scanMode;

        if (!queryToUse) {
            alert("Por favor ingrese o seleccione un valor de búsqueda");
            return;
        }
        setScanning(true);
    ...
    return (
        <DashboardLayout title="Verificaciones Pendientes" subtitle="Revisa y aprueba solicitudes de guías independientes">
            <div className="p-8">
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-6">
                                    ...
                                    <div className="bg-gray-800 rounded-lg p-4">
                                        ...
                                        {selectedVerification.selfie_url ? (
                                            <div className="aspect-square bg-gray-700 rounded-lg overflow-hidden relative border border-gray-600">
                                                <img
                                    ...
                                    <div className="bg-gray-800 rounded-lg p-4">
                                        ...
                                        {selectedVerification.dni_url ? (
                                            <div className="aspect-[16/10] bg-gray-700 rounded-lg overflow-hidden relative border border-gray-600">
                                                <img
                                                    src={selectedVerification.dni_url}
                                                    alt="DNI"
                                                    className="w-full h-full object-cover"
                                                />
                                            </div>
                                        ...
                                        <div className="flex gap-2 mb-4 border-b border-gray-800 pb-4">
                                            {selectedVerification.license_number && (
                                                <>
                                                    <button
                                                        onClick={() => {
                                                            setScanMode('dni-phys');
                                                            setScanInput(selectedVerification.license_number!);
                                                            performDeepScan(selectedVerification.license_number, 'dni-phys');
                                                        }}
                                                        className="px-3 py-1 bg-blue-900/40 text-blue-400 border border-blue-500/50 rounded flex items-center gap-2 text-xs hover:bg-blue-900/60 transition"
                                                    >
                                                        <span>🪪</span> Consultar DNI Físico
                                                    </button>
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            setScanInput(selectedVerification.license_number!);
                                                            performDeepScan(selectedVerification.license_number, 'background');
                                                        }}
                                                        className="px-3 py-1 bg-red-900/40 text-red-400 border border-red-500/50 rounded flex items-center gap-2 text-xs hover:bg-red-900/60 transition"
                                                    >
                                                        <span>🚨</span> Filtrar Antecedentes
                                                    </button>
                                                </>
                                            )}
                                            {selectedVerification.user_phone && (
                                                <button
                                                    onClick={() => {
                                                        setScanMode('phone');
                                                        setScanInput(selectedVerification.user_phone!);
                                                        performDeepScan(selectedVerification.user_phone, 'phone');
                                                    }}
                                                    className="px-3 py-1 bg-green-900/40 text-green-400 border border-green-500/50 rounded flex items-center gap-2 text-xs hover:bg-green-900/60 transition"
                                                >
                                                    <span>📞</span> Revisar Teléfono
                                                </button>
                                            )}
                                        </div>
                                    )}
