...
export default function TrackingPage() {
    ...
    return (
        <DashboardLayout title="Centro de Control" subtitle="Monitoreo GPS en tiempo real">
            <div className="flex-1 flex w-full relative h-[calc(100vh-80px)] overflow-hidden">
                {/* Sidebar - Control Panel */}
                <aside className="w-80 bg-[#1a2235]/95 backdrop-blur-md border-r border-white/10 flex flex-col h-full z-10 shadow-xl relative">
                    {/* Header */}
                    <div className="p-4 border-b border-white/10 shrink-0">
                        <div className="flex items-center justify-between">
                            <h2 className="text-white font-bold flex items-center gap-2">
                                <span className="text-blue-400">📡</span> Estado de Red
                            </h2>
                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-wider ${connected ? 'bg-green-500/20 text-green-400 border border-green-500/30 shadow-[0_0_10px_rgba(34,197,94,0.3)] animate-pulse' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
